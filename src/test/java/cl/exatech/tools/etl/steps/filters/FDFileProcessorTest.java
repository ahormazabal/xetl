    ...
    Stream<Record> dataStream = StreamSupport.stream(csvParser.spliterator(), false)
        .map(line -> {
          ...
          return new MutableRecord()
              ...
              .append(FDFileProcessor.LEVEL_FIELD, "PT")
              .append(FDFileProcessor.CM_FIELD, "RV")
              .append(FDFileProcessor.PT_FIELD, pt)
              .append("cl", "")
              .append("ct", "")
              .append("f", "")
//              .append("riesgot", "")
              .append(FDFileProcessor.SCENARIOS_FIELD, evaluesString);
        });

    // Add empty Data cases.
    Record emptyTemplate = new MutableRecord()
        .append("fecha_proceso", "1979-11-20")
        .append(FDFileProcessor.LEVEL_FIELD, "CL")
        .append(FDFileProcessor.CM_FIELD, "RF")
        .append(FDFileProcessor.PT_FIELD, "35")
        .append("cl", "")
        .append("ct", "")
        .append("f", "")
//        .append("riesgot", "")
        .append(FDFileProcessor.SCENARIOS_FIELD, "[]");
    Stream<Record> emptyData = Stream.of(
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "35"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "20"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "86"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "90"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "92"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "70"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "72"),
        new MutableRecord(emptyTemplate).set(FDFileProcessor.PT_FIELD, "236")
    );
    ...
    Map<String, Map<String, Double>> results = testStream
        .collect(Collectors.groupingBy(r -> r.get(FDFileProcessor.LEVEL_FIELD) + "-" + r.get(FDFileProcessor.CM_FIELD),
            Collectors.toMap(r -> r.get(FDFileProcessor.PT_FIELD), r -> Double.valueOf(r.get(FDFileProcessor.RIESGO_TT_KEY)))
        ));
